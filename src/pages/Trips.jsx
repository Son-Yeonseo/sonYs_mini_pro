  ...
  return (
    <Card title="여행" right={<Link to="/trips/new"><Button>+ 새 여행</Button></Link>}>
      <div className="flex flex-col gap-3">
        ...
        {items.map(t=> (
          <div key={t.id} className="flex items-center justify-between p-4 bg-surface rounded-lg border border-primary-dark/10 hover:shadow-sm transition-shadow">
            <div>
              <div className="font-semibold text-text">{t.name}</div>
              <div className="text-sm text-text-soft">{t.city} · {t.start} ~ {t.end}</div>
            </div>
